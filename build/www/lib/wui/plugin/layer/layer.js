...
Class.pt.tips = function(){
  ...
  goal.where = [function(){ //上        
    ...
    var css = win.width() - (goal.left + goal.width + layArea[0] + 8*2) > 0 ?
      {'border-left-color': config.tips[1],'border-right-color': "transparent"} :
      {'border-right-color': config.tips[1]};
      tipsG.removeClass('layui-layer-TipsB').addClass('layui-layer-TipsT').css(css);
  }, function(){ //右
  ...
  /* 8*2为小三角形占据的空间 */
  if(guide === 1){
    goal.top - (win.scrollTop() + layArea[1] + 8*2) && goal.where[0]()  ;
  } else if(guide === 2){
